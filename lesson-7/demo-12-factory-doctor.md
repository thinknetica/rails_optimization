...
Получить подсказки о неоптимальных вещах типа `create` вместо `build_stubbed`
...
FlareTag (./spec/labor/flare_tag_spec.rb:3)
  ...
  returns nil if an article doesn't have a flare tag (./spec/labor/flare_tag_spec.rb:32) – 3 records created, 00:00.135
...
MailchimpBot (./spec/labor/mailchimp_bot_spec.rb:13)
  returns false if user isn't a sustaining member (./spec/labor/mailchimp_bot_spec.rb:86) – 2 records created, 00:00.122
...
Comment (./spec/models/comment_spec.rb:3)
  ...
  returns the root article's user if no parent comment (./spec/models/comment_spec.rb:158) – 6 records created, 00:00.378
...
ArticlePolicy (./spec/policies/article_policy_spec.rb:3)
  ...
  forbids the first user from viewing the other user's analytics via their article (./spec/policies/article_policy_spec.rb:58) – 5 records created, 00:00.251
  forbids the other user from viewing the first user's analytics (./spec/policies/article_policy_spec.rb:62) – 5 records created, 00:00.252
